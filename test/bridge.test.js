...
test("POST /api/docs emits wicked.doc.created onto the SSE bridge", async () => {
  const bridge = await openBridge();
  try {
    assert.equal((await createDoc("alpha")).status, 200);
    const f = await bridge.waitFor(isType("wicked.doc.created", "alpha"));
...
test("UI feedback.submitted → service applies it → version.created + feedback.processed fan back", async () => {
  ...
  try {
    ...
    const vc = await bridge.waitFor(isType("wicked.version.created", "beta"));
    assert.equal(vc.data.payload.kind, "deterministic");
    assert.equal(vc.data.payload.version, 1);
    const fp = await bridge.waitFor(isType("wicked.feedback.processed", "beta"));
...
test("structural loop: feedback.processed carries fragments → scripted agent emits edit.completed → new version", async () => {
  ...
  try {
    ...
    const fp = await bridge.waitFor(isType("wicked.feedback.processed", "gamma"));
    ...
    await emitEvent("wicked.edit.completed",
    ...
    const vc = await bridge.waitFor((f) => f.ev === "wicked.version.created" && f.data.payload.document_id === "gamma" && f.data.payload.kind === "structural");
...
test("agent draft.completed lands a generated first draft for a source doc", async () => {
  ...
  const bridge = await openBridge();
  try {
    await emitEvent("wicked.draft.completed",
      { document_id: "delta", html: "<section><h1>Investor Update</h1><p>Numbers.</p></section>" },
      { producer: PRODUCERS.AGENT });
    const vc = await bridge.waitFor((f) => f.ev === "wicked.version.created" && f.data.payload.document_id === "delta" && f.data.payload.kind === "generated");
...
test("chat.posted fans to the bridge and persists to the transcript", async () => {
  ...
  const bridge = await openBridge();
  try {
    await jpost("/api/events", { event_type: "wicked.chat.posted", payload: { document_id: "epsilon", role: "user", text: "make it premium" } });
    await bridge.waitFor(isType("wicked.chat.posted", "epsilon"));
...
test("source.attached persists to sources.json (materialized by the command loop)", async () => {
  ...
  const bridge = await openBridge();
  try {
    await jpost("/api/events", { event_type: "wicked.source.attached", payload: { document_id: "zeta", added: [{ path: "/tmp/refs", note: "use" }] } });
    await bridge.waitFor(isType("wicked.source.attached", "zeta"));
...
test("POST /api/events enforces the UI whitelist + known doc", async () => {
  ...
  assert.equal((await jpost("/api/events", { event_type: "wicked.edit.completed", payload: { document_id: "eta", version: 1, results: [] } })).status, 403);
  // Unknown event type:
  assert.equal((await jpost("/api/events", { event_type: "wicked.bogus.happened", payload: { document_id: "eta" } })).status, 400);
  // Unknown doc:
  assert.equal((await jpost("/api/events", { event_type: "wicked.chat.posted", payload: { document_id: "nonesuch", role: "user", text: "hi" } })).status, 404);
});
...
test("idempotency: a re-emitted feedback command does not create a second version", async () => {
  ...
  try {
    ...
    const ev = { event_type: "wicked.feedback.submitted", domain: "wicked-interactive", subdomain: "feedback", payload: { ts: new Date().toISOString(), ...payload }, producer_id: PRODUCERS.UI, idempotency_key: key };
    emit(busDb(), busConfig(), ev);
    await bridge.waitFor(isType("wicked.version.created", "theta"));
