...
The service emits `wicked.interactive.doc.created` which the browser receives via SSE and **automatically
...
```bash
wibus wicked.interactive.chat.posted chat '{"document_id":"<doc>","role":"agent","text":"I'\''m here — describe a change or click any block and I'\''ll get to work."}'
```
...
- **Ignore your own emissions** (`producer_id: "wi-agent"`) and the service's facts
  (`wicked.interactive.version.created`, `wicked.interactive.export.requested`) — the browser handles those.
...
A `wicked.interactive.feedback.processed` line with `awaiting_structural > 0` carries the items inline —
...
### 3c. Verify, then emit `wicked.interactive.edit.completed`
...
execFileSync("wicked-bus", ["emit", "--type", "wicked.interactive.edit.completed", "--domain", "wicked-interactive",
...
`wicked.interactive.version.created` (the browser hot-reloads). If a fragment dropped an anchor, the apply
...
```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"processing","message":"Reworking that card…","version":5}'
# …after the new version lands:
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"complete","message":"Done — updated.","version":6}'
```
...
```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"working","message":"Re-rendering the PDF to check the lane layout…"}'
```

**Format — write messages that scan, not blobs.** The thread renders light **markdown**, so structure
the `message` text of `wicked.interactive.status.posted` (and any `wicked.interactive.chat.posted` reply) instead of shipping a
wall of prose:
...
```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"asking","question":"Two-column or stacked?","options":["Two-column","Stacked"],"request_id":"q1"}'
```

Their choice arrives as a `wicked.interactive.question.answered` event carrying `request_id` + `answer`.
...
On a `wicked.interactive.chat.posted` event with `role: "user"`:

- **Conversational** ("what can you do?", "how do I export?") — reply with a chat post:
  `wibus wicked.interactive.chat.posted chat '{"document_id":"<doc>","role":"agent","text":"…"}'`.
- **A targeted change** ("make the hero bolder", "fix that number") — fetch the current head
  ...
  `wicked.interactive.edit.completed` with `version` set to the **current head** (Step 3c). The service lands
...
- **A whole-document change** ("add a pricing slide", "make the whole thing premium", or the
  ...
  `wicked.interactive.draft.completed` with `{ "document_id", "html" }` (or `html_path` for a large draft,
...
```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"asking","question":"What'\''s this for and who'\''s the audience?","options":["Investor update","Customer pitch","Internal report","Something else"]}'
```
Then wait for `wicked.interactive.question.answered` before generating. This is the right path when intent
is genuinely unclear — the Thread is open and the user is right there.

```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"processing","message":"Reading your brief and drafting…"}'
```
...
wibus wicked.interactive.draft.completed generation "$(node -e 'const fs=require("fs");process.stdout.write(JSON.stringify({document_id:"<doc>",html:fs.readFileSync("/tmp/wi-draft.html","utf8")}))')"
# or for a large draft, pass it by path and skip inlining:
wibus wicked.interactive.draft.completed generation '{"document_id":"<doc>","html_path":"/tmp/wi-draft.html"}'
```

The service lands it as `_v1` and the browser hot-reloads. Post `complete`:

```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"complete","message":"Here'\''s a first draft — click any block to refine it.","version":1}'
```
...
- You remain the **single writer back into the loop**: take the crew's output, preserve every
  `data-wid` (Step 2), and land it via `wicked.interactive.edit.completed` (targeted) or
  `wicked.interactive.draft.completed` (whole-doc). The crew reasons; you satisfy the INV-2 gate.
...
A `wicked.interactive.doc.created` event with `kind: "demo"` points at a **live URL** (in `payload.url`, with
...
```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"working","message":"Exploring the app and working out the click-path…"}'
```
...
1. Read the `brief` from the event payload (or from `wicked.interactive.chat.posted` if the user typed it).
...
5. **Post the scene plan and WAIT for confirmation — do NOT start recording yet.** This is an
   ...
   ```bash
   wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"asking","question":"Does this scene breakdown look right?","options":["Looks good — record it","Let me adjust"]}'
   wibus wicked.interactive.chat.posted chat '{"document_id":"<doc>","role":"agent","text":"Here'\''s how I'\''m breaking this into scenes:\n- **Scene 1: ...**\n- **Scene 2: ...**\n\nDoes this look right? Say '\''looks good'\'' to start recording, or tell me what to change."}'
   ```
6. Wait for `wicked.interactive.question.answered` (button click) or `wicked.interactive.chat.posted` (typed reply)
   confirming the plan. If they want changes, update the scene list and re-confirm. Only proceed
   to Playwright and `wicked.interactive.demo.requested` after explicit approval.

**Do NOT emit `wicked.interactive.demo.requested` before the user confirms the scene plan.** Jumping
...
```bash
wibus wicked.interactive.demo.requested demo '{"document_id":"<doc>"}'
```
...
progress as `wicked.interactive.status.posted` and emits `wicked.interactive.version.created` when the version lands (or a
`wicked.interactive.status.posted` with `state:"error"` if a step throws — fix the selector and emit
`wicked.interactive.demo.requested` again). The storyboard toolbar offers **Download video** and **GIF**.

### 8c. Refine = re-author + re-record

A demo refines through the **same loop**. When the user highlights a step and asks for a change,
it arrives as a `wicked.interactive.feedback.processed` (Step 3) or `wicked.interactive.chat.posted` (Step 4). For a demo,
"make the change" means **edit `demo.spec.mjs`** and emit `wicked.interactive.demo.requested` again — same spec
...
submits it, a `wicked.interactive.chat.posted` arrives with this format:
...
3. Emit `wicked.interactive.demo.requested` — the service re-runs the full spec but the new step is the
...
3. Emit `wicked.interactive.demo.requested`. The service records a clean take from start to finish.

Either way, post a `complete` status with the scene count when the new version lands.

```bash
wibus wicked.interactive.status.posted status '{"document_id":"<doc>","state":"complete","message":"Done — added \"<scene title>\" (now <N> scenes).","version":<v>}'
```

If you have a question before starting — unclear flow, missing credentials, ambiguous
scope — **ask via `wicked.interactive.status.posted` with `state: "asking"`** (Step 3d). The question
renders as buttons in the UI lock; the user's answer arrives as `wicked.interactive.question.answered`.
...
the URL to a PDF and announced it as `wicked.interactive.theme.learned` with `render_path` (a PDF in the doc
...
file on their own machine; the service does **not** grab anything — it emits `wicked.interactive.theme.learned`
...
- **Tag review progress as a review** so the UI never veils/locks the canvas for it. On any
  `wicked.interactive.status.posted` you emit *for a review*, set `review: true`:
...
two. Narrate start/finish with `wicked.interactive.status.posted` carrying `review:true` (`state:"working"` →
