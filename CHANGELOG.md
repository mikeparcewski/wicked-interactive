...
- **Export honours the author's page geometry — plain `<section>`s are not a slide deck**
  ...
  printed the same way from any browser. Now: (1) an author-declared `@page`, `.page`/`.wi-page`
  wrappers or `break-after: page` are never overridden, and a doc created with `style`
  `web`/`doc`/`brochure` always exports as a document; (2) a deck must DECLARE itself —
  `[data-slide]` / `.slide` / `.wi-slide` markers at document level, `data-wi-kind="deck"`, or
  `style: "ppt"` — plain semantic sections never do; (3) the HTML export carries no print
  ...
  `layout_source`, `page_size` (measured from the PDF, e.g. `A4 portrait`) and `pages` so the
  UI can show what was produced. Behaviour change for undeclared decks: a deck built from plain
